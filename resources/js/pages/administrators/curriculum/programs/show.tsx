...
type ProgramPayload = {
    ...
    ched_authority_serial: string | null;
    ched_authority_year: number | null;
    ched_authority_other_program: string | null;
    ched_delivery_mode: string | null;
    ched_normal_length_years: string | number | null;
...
type ChedOption = { value: string; label: string };
...
export default function CurriculumProgramShow({
    ...
    const programForm = useForm({
        ...
        catalog_reference: program.catalog_reference ?? "",
        duration_hours: fmt(program.duration_hours),
        qualification_level: program.qualification_level ?? "",
        tesda_program_type: program.tesda_program_type ?? "national_certificate",
        duration_years: fmt(program.duration_years),
        internship_hours: fmt(program.internship_hours),
    ...
    const handleProgramSubmit = (e: FormEvent) => {
        e.preventDefault();
        programForm.put(route("administrators.curriculum.programs.update", program.id), { preserveScroll: true });
    };
    ...
    const handleEditSubject = (e: FormEvent) => {
        e.preventDefault();
        if (!editSubject) return;
        editForm.put(route("administrators.curriculum.programs.subjects.update", { course: program.id, subject: editSubject.id }), {
            preserveScroll: true,
            onSuccess: () => {
                setEditSubject(null);
                editForm.reset();
    ...
    const openEditSubject = (s: SubjectPayload) => {
        editForm.setData({
            code: s.code,
            title: s.title,
            classification: s.classification ?? defaultSubject.classification,
            units: fmt(s.units),
            lecture: fmt(s.lecture),
            laboratory: fmt(s.laboratory),
            academic_year: s.academic_year?.toString() ?? "",
            semester: s.semester?.toString() ?? "",
            group: s.group ?? "",
            is_credited: s.is_credited,
            pre_riquisite: s.pre_riquisite ?? [],
        });
        editForm.clearErrors();
        setEditSubject(s);
    };
    const handleDeleteSubject = () => {
        ...
        router.delete(route("administrators.curriculum.programs.subjects.destroy", { course: program.id, subject: deleteSubject.id }), {
            preserveScroll: true,
    ...
    const renderPrereqs = (s: SubjectPayload) => {
        ...
        return (
            <div className="flex flex-wrap gap-1">
                {s.pre_riquisite.map((id) => {
                    const o = subjectOptionMap.get(id);
                    return (
                        <Badge key={id} variant="outline" className="bg-muted/50 font-mono text-[10px]" title={o ? o.title : `#${id}`}>
                            {o ? o.code : `#${id}`}
                        </Badge>
                    );
                })}
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="flex flex-col gap-1">
                            <span className="text-foreground font-semibold tracking-tight">{s.code}</span>
                            <span className="text-muted-foreground max-w-xs truncate text-xs">{s.title}</span>
                            <div className="mt-1 flex flex-wrap gap-1">
                                <Badge variant="secondary" className="h-4 px-1.5 text-[10px] font-medium">
                                    {s.classification ? (classificationMap.get(s.classification) ?? s.classification) : "Unassigned"}
                                </Badge>
                                {s.is_credited && (
                                    <Badge
                                        variant="outline"
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <div className="flex flex-col">
                        <span className="text-foreground text-sm font-medium">Year {row.original.academic_year ?? "-"}</span>
                        <span className="text-muted-foreground text-xs">Sem {row.original.semester ?? "-"}</span>
                    </div>
                ),
            },
            {
                ...
                cell: ({ row }) => (
                    <div className="flex flex-col">
                        <span className="text-foreground text-sm font-medium">{row.original.units ?? 0} Units</span>
                        <span className="text-muted-foreground text-xs">
                            Lec: {row.original.lecture ?? 0} | Lab: {row.original.laboratory ?? 0}
                        </span>
                    </div>
                ),
            },
                ...
                cell: ({ row }) => (
                    <div className="flex justify-end gap-1">
                        <Button
                            ...
                            onClick={() => openEditSubject(row.original)}
                            title="Edit"
                        >
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant="outline" className="bg-background px-2.5 py-0.5 text-xs">
                            ...
                            <CardHeader className="bg-muted/20 border-b pb-4">
                                <div className="flex flex-col justify-between gap-4 sm:flex-row">
                                    <div>
                                        ...
                                        <CardDescription>Manage subjects for this program.</CardDescription>
                                    </div>
                                        ...
                                        {table.getRowModel().rows?.length ? (
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={columns.length} className="text-muted-foreground h-32 text-center">
                                                    {subjects.length === 0
                                                        ? "No subjects yet. Add the first subject."
                    ...
                    <TabsContent value="settings">
                        <Card className="border shadow-sm">
                            <CardHeader>
                                ...
                                <CardDescription>Update program details, department assignment, and fee configuration.</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <form className="grid gap-6" onSubmit={handleProgramSubmit}>
                                    <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
                                        <div className="grid gap-2 lg:col-span-1">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => programForm.setData("code", e.target.value)}
                                            />
                                        ...
                                        <div className="grid gap-2 lg:col-span-3">
                                            ...
                                            <Input
                                                ...
                                                className="font-medium"
                                                onChange={(e) => programForm.setData("title", e.target.value)}
                                            />
                                        ...
                                        <div className="grid gap-2 lg:col-span-2">
                                            ...
                                            <Select
                                                value={programForm.data.department_id}
                                                onValueChange={(v) => programForm.setData("department_id", v)}
                                            >
                                                <SelectTrigger>
                                        ...
                                        <div className="grid gap-2 lg:col-span-2">
                                            ...
                                            <Select
                                                value={programForm.data.course_type_id}
                                                onValueChange={(v) => programForm.setData("course_type_id", v)}
                                            >
                                                <SelectTrigger>
                                        ...
                                        <div className="grid gap-2 lg:col-span-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => programForm.setData("curriculum_year", e.target.value)}
                                            />
                                        ...
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => programForm.setData("lec_per_unit", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.lec_per_unit} />
                                        </div>
                                        <div className="grid gap-2 lg:col-span-2">
                                            <Label className="text-muted-foreground text-xs font-semibold tracking-wider uppercase">Misc Fee</Label>
                                            <Input
                                                type="number"
                                                value={programForm.data.miscelaneous}
                                                onChange={(e) => programForm.setData("miscelaneous", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.miscelaneous} />
                                        </div>
                                    </div>
                                    {programForm.data.curriculum_kind === "tesda_qualification" && (
                                        <div className="border-border/70 grid gap-4 rounded-lg border p-4 md:grid-cols-2">
                                            ...
                                            <div className="grid gap-2">
                                                ...
                                                <Select
                                                    value={programForm.data.tesda_program_type}
                                                    onValueChange={(value) => programForm.setData("tesda_program_type", value)}
                                                >
                                                    <SelectTrigger>
                                                ...
                                                <FieldError message={programForm.errors.tesda_program_type} />
                                            </div>
                                            <div className="grid gap-2">
                                                <Label>Qualification level</Label>
                                                <Input
                                                    value={programForm.data.qualification_level}
                                                    placeholder="e.g. NC II or Diploma"
                                                    onChange={(e) => programForm.setData("qualification_level", e.target.value)}
                                                />
                                                <FieldError message={programForm.errors.qualification_level} />
                                            </div>
                                            <div className="grid gap-2">
                                                <Label>Total program hours</Label>
                                                <Input
                                                    type="number"
                                                    min="1"
                                                    value={programForm.data.duration_hours}
                                                    onChange={(e) => programForm.setData("duration_hours", e.target.value)}
                                                />
                                                <FieldError message={programForm.errors.duration_hours} />
                                            </div>
                                            {programForm.data.tesda_program_type === "diploma" && (
                                                <>
                                                    <div className="grid gap-2">
                                                        <Label>Typical duration (years)</Label>
                                                        <Input
                                                            type="number"
                                                            min="0.5"
                                                            max="10"
                                                            step="0.5"
                                                            value={programForm.data.duration_years}
                                                            onChange={(e) => programForm.setData("duration_years", e.target.value)}
                                                        />
                                                        <FieldError message={programForm.errors.duration_years} />
                                                    </div>
                                                    <div className="grid gap-2">
                                                        <Label>Mandatory OJT / internship hours</Label>
                                                        <Input
                                                            type="number"
                                                            min="0"
                                                            max="65535"
                                                            value={programForm.data.internship_hours}
                                                            onChange={(e) => programForm.setData("internship_hours", e.target.value)}
                                                        />
                                                        <FieldError message={programForm.errors.internship_hours} />
                                                    </div>
                                                    <div className="grid gap-2 md:col-span-2">
                                                        <Label>Bundled TESDA qualifications</Label>
                                                        <Input
                                                            placeholder="e.g. Cookery NC II, Bread & Pastry Production NC II"
                                                            value={programForm.data.bundled_qualifications}
                                                            onChange={(e) => programForm.setData("bundled_qualifications", e.target.value)}
                                                        />
                                                        <p className="text-muted-foreground text-xs">Separate each qualification with a comma.</p>
                                                        <FieldError message={programForm.errors.bundled_qualifications} />
                                                    </div>
                                                    <div className="grid gap-2 md:col-span-2">
                                                        <Label>Advanced topics and internship context</Label>
                                                        <Textarea
                                                            className="min-h-[80px] resize-none"
                                                            value={programForm.data.advanced_topics}
                                                            onChange={(e) => programForm.setData("advanced_topics", e.target.value)}
                                                        />
                                                        <FieldError message={programForm.errors.advanced_topics} />
                                                    </div>
                                                </>
                                            )}
                                        </div>
                                    ...
                                    <div className="grid gap-5 md:grid-cols-2">
                                        <div className="grid gap-2">
                                            ...
                                            <Textarea
                                                ...
                                                onChange={(e) => programForm.setData("description", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.description} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label className="text-muted-foreground text-xs font-semibold tracking-wider uppercase">Remarks</Label>
                                            <Textarea
                                                className="min-h-[100px] resize-none"
                                                value={programForm.data.remarks}
                                                onChange={(e) => programForm.setData("remarks", e.target.value)}
                                            />
                                    ...
                                    <div className="border-border/70 grid gap-4 rounded-lg border p-4 md:grid-cols-2 lg:grid-cols-4">
                                        <div className="col-span-full">
                                            <p className="text-sm font-semibold">CHED Form B/C program profile</p>
                                            <p className="text-muted-foreground text-xs">
                                        ...
                                        <div className="grid gap-2">
                                            <Label>Major</Label>
                                            <Input
                                                value={programForm.data.ched_major}
                                                onChange={(e) => programForm.setData("ched_major", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_major} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Thesis / Dissertation</Label>
                                            <Select
                                                value={programForm.data.ched_has_thesis || undefined}
                                                onValueChange={(value) => programForm.setData("ched_has_thesis", value === "__unknown" ? "" : value)}
                                            >
                                                <SelectTrigger>
                                                ...
                                                <SelectContent>
                                                    ...
                                                    {ched_options.has_thesis.map((option) => (
                                            ...
                                            <FieldError message={programForm.errors.ched_has_thesis} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Program status</Label>
                                            <Select
                                                value={programForm.data.ched_program_status || undefined}
                                                onValueChange={(value) =>
                                                    programForm.setData("ched_program_status", value === "__unknown" ? "" : value)
                                                }
                                            >
                                                <SelectTrigger>
                                                    <SelectValue placeholder="Unknown" />
                                                ...
                                                <SelectContent>
                                                    <SelectItem value="__unknown">Unknown</SelectItem>
                                                    {ched_options.program_statuses.map((option) => (
                                                        <SelectItem key={option.value} value={option.value}>
                                                            {option.label}
                                                        </SelectItem>
                                                    ))}
                                            ...
                                            <FieldError message={programForm.errors.ched_program_status} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Delivery mode</Label>
                                            <Select
                                                value={programForm.data.ched_delivery_mode || undefined}
                                                onValueChange={(value) =>
                                                    programForm.setData("ched_delivery_mode", value === "__unknown" ? "" : value)
                                                }
                                            >
                                                <SelectTrigger>
                                                    <SelectValue placeholder="Unknown" />
                                                ...
                                                <SelectContent>
                                                    <SelectItem value="__unknown">Unknown</SelectItem>
                                                    {ched_options.delivery_modes.map((option) => (
                                                        <SelectItem key={option.value} value={option.value}>
                                                            {option.label}
                                                        </SelectItem>
                                                    ))}
                                            ...
                                            <FieldError message={programForm.errors.ched_delivery_mode} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Authority category</Label>
                                            <Select
                                                value={programForm.data.ched_authority_category || undefined}
                                                onValueChange={(value) =>
                                                    programForm.setData("ched_authority_category", value === "__unknown" ? "" : value)
                                                }
                                            >
                                                <SelectTrigger>
                                                    <SelectValue placeholder="Unknown" />
                                                ...
                                                <SelectContent>
                                                    <SelectItem value="__unknown">Unknown</SelectItem>
                                                    {ched_options.authority_categories.map((option) => (
                                                        <SelectItem key={option.value} value={option.value}>
                                                            {option.label}
                                                        </SelectItem>
                                                    ))}
                                            ...
                                            <FieldError message={programForm.errors.ched_authority_category} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Authority serial</Label>
                                            <Input
                                                value={programForm.data.ched_authority_serial}
                                                onChange={(e) => programForm.setData("ched_authority_serial", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_authority_serial} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Authority year</Label>
                                            <Input
                                                type="number"
                                                value={programForm.data.ched_authority_year}
                                                onChange={(e) => programForm.setData("ched_authority_year", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_authority_year} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Other program reference</Label>
                                            <Input
                                                value={programForm.data.ched_authority_other_program}
                                                onChange={(e) => programForm.setData("ched_authority_other_program", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_authority_other_program} />
                                        </div>
                                        <div className="grid gap-2">
                                            <Label>Normal length (years)</Label>
                                            <Input
                                                type="number"
                                                min="0"
                                                max="999.9"
                                                step="0.5"
                                                value={programForm.data.ched_normal_length_years}
                                                onChange={(e) => programForm.setData("ched_normal_length_years", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_normal_length_years} />
                                        </div>
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                min="0"
                                                max="65535"
                                                value={programForm.data.ched_program_credit_units}
                                                onChange={(e) => programForm.setData("ched_program_credit_units", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_program_credit_units} />
                                        </div>
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                max="9999999999.99"
                                                step="0.01"
                                                value={programForm.data.ched_tuition_per_unit}
                                                onChange={(e) => programForm.setData("ched_tuition_per_unit", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_tuition_per_unit} />
                                        </div>
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                max="9999999999.99"
                                                step="0.01"
                                                value={programForm.data.ched_program_fee}
                                                onChange={(e) => programForm.setData("ched_program_fee", e.target.value)}
                                            />
                                            <FieldError message={programForm.errors.ched_program_fee} />
                                        </div>
                                    </div>
