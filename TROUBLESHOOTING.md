...
Verify Redis health, `QUEUE_CONNECTION=redis`, and the application logs. Horizon workers run inside the application container in the production image. Avoid repeatedly dispatching the same export or notification until the original job state is understood.
