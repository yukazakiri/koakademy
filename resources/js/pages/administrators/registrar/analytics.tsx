...
export default function RegistrarAnalytics({ user, analytics, quality, report, generatedAt }: Props) {
    ...
    return <AdminLayout user={user} title="Registrar Analytics">
        ...
        <div className="space-y-6 pb-12">
            ...
            <Card><CardHeader><CardTitle className="text-base">Enrollment by program and year level</CardTitle><CardDescription>Every program is shown with its selected reporting-population count for each year level.</CardDescription></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[1100px] text-xs"><thead className="bg-muted/60 text-left"><tr><th className="p-2">Department</th><th className="p-2">Program code</th><th className="p-2">Program title</th>{['First','Second','Third','Fourth','Fifth','Sixth','Seventh'].map((year) => <th key={year} className="p-2">{year} year</th>)}<th className="p-2">Total</th></tr></thead><tbody>{analytics.program_year_matrix.length === 0 ? <tr><td colSpan={11} className="text-muted-foreground p-6 text-center">No programs match the report filters.</td></tr> : analytics.program_year_matrix.map((row, rowIndex) => <tr key={`${row.program_code}-${rowIndex}`} className="border-b"><td className="p-2">{row.department ?? 'Unassigned'}</td><td className="p-2 font-medium">{row.program_code ?? 'Unassigned'}</td><td className="p-2">{row.program_title ?? 'Unassigned program'}</td>{[1,2,3,4,5,6,7].map((year) => <td key={year} className="p-2">{Number(row[`year_${year}`] ?? 0)}</td>)}<td className="p-2 font-semibold">{Number(row.total ?? 0)}</td></tr>)}</tbody></table></CardContent></Card>
