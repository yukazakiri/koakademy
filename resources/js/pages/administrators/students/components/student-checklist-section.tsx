import { Badge, Badge as ReuiBadge } from "@/components/ui/badge";
...
function GwaSummary({ label, result, className }: GwaSummaryProps) {
    ...
    return (
        <div className={`bg-muted/30 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-md border px-3 py-2 text-sm ${className ?? ""}`}>
                    ...
                    return (
                        <TabsContent key={yearGroup.year} value={String(yearGroup.year)}>
                            <div className="space-y-6">
                                {yearResult && <GwaSummary label={`${getYearLabel(yearGroup.year)} GWA`} result={yearResult} />}
                                                    ...
                                                    {semesterResult && (
                                                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
                                                            <span className="flex items-baseline gap-1">
                                                                <span className="text-muted-foreground text-xs uppercase">GWA</span>
                                                                <span
                                                                    className={`font-mono text-sm font-bold ${gwaToneClass(semesterResult, gradingConfig)}`}
                                                                >
                                                                            ...
                                                                            return (
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    <TableCell className="text-muted-foreground text-sm italic">
                                                                                        ...
                                                                                        {subject.title} - SY {history.school_year} (Sem{" "}
                                                                                        {history.semester})
                                                                                    </TableCell>
