...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .purpose { margin: 18px 0; padding: 9px 11px; border-left: 3px solid #9b6b32; background: #fbf8f2; }
        ...
        .variant-institutional .masthead { flex-direction: column; justify-content: center; text-align: center; border-bottom-color: #9b6b32; }
        .variant-institutional .document-heading h1 { color: #9b6b32; }
        .variant-compact { font-size: 9pt; }
        .variant-compact .masthead { padding-bottom: 6px; }
        .variant-compact .school-name { font-size: 13pt; }
        .variant-compact .document-heading { margin-top: 11px; }
        .variant-compact .student-grid { gap: 4px 12px; margin-bottom: 10px; }
        .variant-compact table { font-size: 8pt; margin-top: 7px; margin-bottom: 10px; }
        .variant-compact th, .variant-compact td { padding: 4px 5px; }
        .variant-advising .document-heading h1 { color: #1d4ed8; }
        .variant-advising .masthead { border-bottom-color: #1d4ed8; }
        .variant-detailed .document-heading h1 { color: #1d4ed8; }
        .variant-detailed .masthead { border-bottom-color: #1d4ed8; }
    </style>
</head>
<body>
@php
    ...
    $variant = (string) ($data['variant'] ?? 'classic');
    $grades = data_get($data, 'grades.subjects', []);
    $subjects = $enrollment['subjects'] ?? [];
@endphp
<main class="document variant-{{ in_array($variant, ['classic', 'institutional', 'compact', 'standard', 'advising', 'official', 'detailed'], true) ? $variant : 'classic' }}">
    ...
    @if ($template === 'certificate_of_enrollment')
        ...
        @if (count($subjects) > 0)
            <table>
                <thead><tr><th>Code</th><th>Descriptive title</th><th>Section</th><th class="number">Units</th></tr></thead>
                <tbody>
                @foreach ($subjects as $subject)
                    <tr><td>{{ $subject['code'] }}</td><td>{{ $subject['title'] }}</td><td>{{ $subject['section'] }}</td><td class="number">{{ $subject['units'] }}</td></tr>
                @endforeach
                </tbody>
            </table>
    ...
    @elseif ($template === 'registration_form')
        ...
        @if (count($subjects) > 0)
            <table>
                <thead><tr><th>No.</th><th>Code</th><th>Descriptive title</th><th>Section</th><th>Schedule</th><th class="number">Units</th></tr></thead>
                <tbody>
                @foreach ($subjects as $index => $subject)
                    <tr><td>{{ $index + 1 }}</td><td>{{ $subject['code'] }}</td><td>{{ $subject['title'] }}</td><td>{{ $subject['section'] }}</td><td>{{ $subject['schedule'] }}</td><td class="number">{{ $subject['units'] }}</td></tr>
                @endforeach
                </tbody>
            </table>
    ...
    @elseif ($template === 'grade_report')
        ...
        @if (count($grades) > 0)
            <table>
                <thead><tr><th>Code</th><th>Descriptive title</th><th class="number">Units</th><th class="number">Prelim</th><th class="number">Midterm</th><th class="number">Finals</th><th class="number">Average</th><th>Status</th></tr></thead>
                <tbody>
    ...
    <footer class="footer"><span>Generated {{ $data['generated_at'] ?? '' }}</span><span>Official registrar document</span></footer>
</main>
</body>
</html>
