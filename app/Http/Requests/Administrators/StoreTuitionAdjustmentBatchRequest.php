...
final class StoreTuitionAdjustmentBatchRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'rows.*.gross_lecture' => ['nullable', 'numeric', 'min:0'],
            'rows.*.calculation_method' => ['nullable', 'string', 'in:reconciled_assessment,fee_rates'],
            'rows.*.fee_schedule_version_id' => ['nullable', 'integer', 'exists:fee_schedule_versions,id'],
            'rows.*.calculation_inputs' => ['nullable', 'array'],
            'rows.*.fingerprint' => ['nullable', 'string', 'max:64'],
