...
final readonly class LegacyEnrollmentWorkflowAdapter
{
        ...
        try {
            ...
            foreach ($separateTransactionFees as $separateTransactionFee) {
                ...
                AdminTransaction::query()->create([
                    ...
                    'amount' => $separateTransactionFee->amount,
                    'type' => 'credit',
                    'description' => 'Enrollment additional fee payment',
                    'status' => $separateTransaction->status,
                ]);
    ...
    public function undoCashierVerification(int $enrollmentRecordId): bool
    {
        ...
        try {
            ...
            if ($linkedStudentTransactions->isEmpty()) {
                $hasAmbiguousLegacyPayments = StudentTransaction::query()
                    ->where('student_id', $enrollmentRecord->student_id)
                    ->whereNull('student_enrollment_id')
                    ->whereHas('transaction', fn ($query) => $query->forAcademicPeriod(
                        str_replace(' ', '', (string) $enrollmentRecord->school_year),
                        (int) $enrollmentRecord->semester,
                    ))
                    ->exists();
                if ($hasAmbiguousLegacyPayments) {
                    throw new Exception(
                        'Legacy payments cannot be reversed automatically because no unambiguous enrollment link exists.',
                    );
                }
            }
            ...
            if ($transactionIds->isNotEmpty()) {
                StudentTransaction::query()->where('student_enrollment_id', $enrollmentRecord->id)->delete();
                AdminTransaction::query()->whereIn('transaction_id', $transactionIds->all())->delete();
                Transaction::query()
                    ->whereIn('id', $transactionIds->all())
                    ->whereDoesntHave('studentTransactions')
                    ->delete();
            }
