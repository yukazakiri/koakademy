...
`.github/workflows/ci.yml` runs for pull requests into `master` and explicit manual dispatches. It does not repeat the full suite after merge; a `master` push starts Delivery directly.
...
Delivery owns the only multi-architecture container build. The path-filtered documentation deployment owns the Astro production build, while CI still catches generated-document and local-link drift before merge.
...
- **Release ordering:** a same-repository `master` push starts Delivery immediately; CI is not repeated after merge.
