...
Browser
  ...
  -> KoAkademy (Laravel + nginx/PHP-FPM + workers + scheduler)
...
Exports, notifications, PDFs, and other expensive operations should use queues. Redis backs queues and Horizon manages worker processes in the production image. Jobs must be retry-safe where possible and should not expose sensitive payloads in logs or failure UIs.
